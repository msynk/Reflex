...
/// </summary>
public sealed class BlexActionContext
{
    private readonly Func<JsonObject>? _capture;
    private JsonObject? _globalState;

    internal BlexActionContext(IStore store, string actionName, Func<JsonObject> captureGlobalState, int sequence, IReadOnlyList<ActionArg>? args)
    {
        Store = store;
...
/// </summary>
public sealed class BlexPreActionContext
{
    internal BlexPreActionContext(IStore store, string actionName, IReadOnlyList<ActionArg>? args)
    {
        Store = store;
        ActionName = actionName;
        Args = args ?? [];
...
public interface IBlexMiddleware
{
    ...
    /// <summary>
    /// Invoked before an action runs. Override to inspect or <see cref="BlexPreActionContext.Cancel">veto</see>
...
public sealed class DelegateMiddleware : IBlexMiddleware
{
    private readonly Action<BlexActionContext> _handler;

    /// <summary>Creates a middleware from a delegate.</summary>
    public DelegateMiddleware(Action<BlexActionContext> handler) => _handler = handler;

    /// <inheritdoc />
    public void OnAction(BlexActionContext context) => _handler(context);
}

/// <summary>
/// A middleware that can veto actions before they run. The delegate returns <c>false</c> to cancel.
/// </summary>
public sealed class FilterMiddleware : IBlexMiddleware
{
    private readonly Func<BlexPreActionContext, bool> _filter;

    /// <summary>Creates a filter; return <c>false</c> from <paramref name="filter"/> to veto the action.</summary>
    public FilterMiddleware(Func<BlexPreActionContext, bool> filter) => _filter = filter;
