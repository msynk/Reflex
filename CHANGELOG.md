...
All notable changes to Reflex are documented here.
...
- **Action payloads**: generated action/effect wrappers capture their arguments as `ActionArg`s,
  visible on `ReflexActionContext.Args` / `ReflexPreActionContext.Args` (payload-aware veto
  filters), in `Reflex.Testing`'s `ActionLog`, and as the `payload` of the action objects streamed
...
- **Persistence hardening**: corrupt payloads are reported, discarded and removed instead of
  ...
  (`ReflexManager.StateRestored`).
- **Error hook**: `options.OnError` / `ReflexManager.OnError` receives every isolated pipeline
  failure (`ReflexError` with source, exception, detail) instead of ad-hoc `Console.Error` writes.
...
- **Manager**: `Unregister(store)`, `DisconnectDevTools()`, lazily-captured
  `ReflexActionContext.GlobalState` (observers that never read the tree no longer pay for
  full-tree serialization).
- **DI**: stores registered via `AddReflexStore<T>()` attach to the manager on first resolution,
  so Reflex works without `<ReflexProvider>` (console apps, workers, tests).
- **Blazor**: `ReflexComponentBase.OwnsSubscription(...)` ties any subscription token to the
  component's lifetime; `Dispose(bool)` is overridable for derived cleanup.
- **Generator diagnostics** REFLEX008-REFLEX016: static/readonly members, `Latest` without a
...
- `ReflexHistory`'s pipeline handlers are exception-isolated: a throwing `Changed` (UI) handler
...
- Generator: nullable-enum default parameters (`Mode? m = Mode.B`) emitted uncompilable code; a
  ...
  comments; a REFLEX005 collision emitted the broken source alongside the diagnostic (now the
...
- Lazy `GlobalState` snapshots are frozen before a reactor's follow-up action mutates state, so
  every observer sees the state that belonged to *its* action; `ReflexHistory` also refreshes its
...
- `StatePersistor`: debounced saves stay on the captured synchronization context and all
  ...
  recognized in its exact shape (no collision with user state containing `__reflexVersion`).
- `ReflexProvider` no longer starts persistence twice concurrently when Blazor renders during
...
- `<ReflexProvider>` no longer fails app startup under Blazor Server prerendering; browser-storage
...
- Sample Counter page no longer leaks a `ReflexHistory.Changed` subscription per visit.
